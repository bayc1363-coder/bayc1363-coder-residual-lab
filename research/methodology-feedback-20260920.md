...
This cell reviews **methods and tests**, not Earth / markets / political substance. It does not re-run overnight RD.
...
## Where they agree (do not upgrade)

- Split RD / Frame Lab / Stage 2 / PCT. Mixing them would manufacture success or failure.
- 50/50 is still a fake-equal risk. RD’s win rule sits in tension with the rest of the desk.
- Stage 2 cannot corroborate Stage 1 while it is Luna reading Luna.
- Self-audit 0.8x is not a scorer.
- PCT stays quarantined.
- Magnet stays out. Prestige×info stays HOLD.
- Keep t2 inhabit / no-reprint. Keep educate ≠ persuade.
- Narrow the live sentence to: **under these prompts, Luna can install two frames and grow them**. Not: OFF proves residualisation, ON removes it.

---

## Where they differ

**Hole.** Luna treats n=1 as making the demand-characteristic hole *decisive*. MiniMax treats n=1 as the hole itself. Both matter. Repeats would not fix a compliance confound; a better construct test would still need repeats.

**What would count against residualisation as operationalised.** Luna is usable: blinded raters find no OFF asymmetry once evidence and task are matched; ON adds labels but not a live investigative move. MiniMax’s first “count against” item (RD skew stays 0.5 across seeds) is **backwards** for residualisation: that is the current overnight FAIL, and it is evidence *for* unmoved canonical pick, not against the construct. It would count against the *thin ON cue*, which the desk already does not equate with equal-res.

**Stage 2.** MiniMax describes Stage 2 as a test of whether education moves Shape more than Decision. That is a finding from the micro, not the instrument’s job. Luna is cleaner: Stage 2 is an informed reflection layer; keep it only if labelled non-independent.

**Blind 1–10.** Luna: drop as residualisation evidence (no mapping). MiniMax: drop self-audit characterisation of “milder.” Same direction.

**Cross-model.** MiniMax wants at least one Frame Lab cell on a second family. Luna wants blinded raters who do not see ON/OFF or the lab vocabulary. Neither is a live EqualResolution claim. Neither is run here.
